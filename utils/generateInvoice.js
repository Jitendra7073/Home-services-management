...
/**
 * Generates invoice PDF from invoice data
...
function generateInvoicePDF(data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // 1️⃣ HEADER SECTION - Two columns
      doc
        .font("Helvetica-Bold")
        .fontSize(18)
        .fillColor("#000")
      ...
      // Ensure we move past header content
      doc.y = Math.max(doc.y, margin + 70);
      drawLine(doc, margin, rightEdge);

      // 2️⃣ CUSTOMER & PROVIDER SECTION - Two columns
      doc.moveDown(0.5);
      ...
      // Calculate customer section height
      const customerEndY = doc.y;

      // Provider (Right) - Reset Y position
      ...
      // Calculate provider section height
      const providerEndY = doc.y;

      // Set Y to whichever section is taller
      doc.y = Math.max(customerEndY, providerEndY);

      doc.moveDown(0.5);
      drawLine(doc, margin, rightEdge);

      // 3️⃣ BOOKING DETAILS TABLE (Only if time exists)
